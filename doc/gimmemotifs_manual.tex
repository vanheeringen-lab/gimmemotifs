...
GimmeMotifs runs on Linux. Definitely not on Windows, sorry. Mac OS X should work in theory, but as I don't have the option to test this, I'm not completely sure. I have tried to make installation of GimmeMotifs as easy as possible. There are installation packages available for Fedora and Ubuntu. It's also possible to install GimmeMotifs from source, but it depends on quite some external packages (motif prediction tools!). Please make sure all prerequisites are installed before installing GimmeMotifs from source. 

\subsection{Installation package}
