    ...
    <head>
      ...
      <script type="text/javascript">
        ...
        function set_data(start, width, show_rc_only, data, annobox) {
          ...
          var num_per_wrap = <xsl:choose><xsl:when test="/mast/model/translate_dna/@value = 'y'">5</xsl:when><xsl:otherwise>4</xsl:otherwise></xsl:choose>;
          var end = start + width;
      ...
      <xsl:if test="/mast/model/translate_dna/@value = 'y'">
        ...
        function Sequence(seqid) {
          ...
          var lines = mysegs.value.split(/\n/);
          for (var i in lines) {
            var line = lines[i];
            var chunks = line.split(/\t/);
            if (chunks.length != 2) continue;
            var start = chunks[0];
            var segment = chunks[1];
            this.segs.push(new Seg(start, segment));
          }
        ...
        <table class="padded-table" border="0" >
          ...
          <thead>
            ...
            <tr>
              ...
              <xsl:for-each select="/mast/motifs/motif">
                <th>Motif <xsl:value-of select="@name" /></th>
              </xsl:for-each>
            </tr>
          </thead>
          <tbody>
      ...
      <xsl:for-each select="/mast/sequences/sequence">
        <xsl:variable name="segs">
          ...
          <xsl:for-each select="seg"> 
            ...
            <xsl:value-of select="translate(data, ' &newline;&tab;', '')" />
            <xsl:text>&newline;</xsl:text>
          </xsl:for-each>
        </xsl:variable>
