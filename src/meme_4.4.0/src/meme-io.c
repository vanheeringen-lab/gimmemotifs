...
static double parse_double(FILE *fp, BUF_T *buf, char *desc) {
  ...
  if (*end != '\0') {
    die("Error reading %s, some of token wasn't number\n", desc);
  }
  if (errno == ERANGE) {
    //Ignore error range, it seems more sensible than choking on really small e-values
    errno = 0;
  }
  return value;
}
 ...
 ***********************************************************************/
static int read_background(const char *bg_filename, const char *meme_filename, ARRAY_T** background) {
...
static BOOLEAN_T read_motif(
  ...
  found = buf_fread_until(buffer, infile, NULL, 2, landmarks+2);
  ...
  found = find_bm_landmarks(infile, buffer, TRUE, FALSE, 2, landmarks);
  do {
    switch (found) {
    case 1:
    ...
    found = find_bm_landmarks(infile, buffer, FALSE, FALSE, 4, landmarks);
  } while (found != 0 && found <= 2);//if next motif or eof then stop searching
...
static void read_motifs(
  ...
  landmarks[2] = bmstr_create("MOTIF ");
  landmarks[3] = bmstr_create(END_OF_MOTIF);

  *num_motifs = 0;
  while (read_motif(motif_file, buffer, landmarks, is_html, background, pseudocount, loading_mode, &(motifs[*num_motifs]))) {
    (*num_motifs)++;
  }
...
static void read_motifs_from_xml(
  ...
  *num_motifs = (xpath_obj->nodesetval ? xpath_obj->nodesetval->nodeNr : 0);
  ...
  for (i_motif = 0; i_motif < *num_motifs; i_motif++) {
    ...
    //set some defaults
    motifs[i_motif].freqs = NULL;
    motifs[i_motif].scores = NULL;
    ...
    if (loading_mode & NEED_PSSM && motifs[i_motif].scores == NULL) {
      die("Motif %s missing PSSM.\n", motifs[i_motif].id);
    }
    if (loading_mode & NEED_PSPM && motifs[i_motif].freqs == NULL) {
      die("Motif %s missing PSPM.\n", motifs[i_motif].id);
    }
...
static BOOLEAN_T read_motif_occurrences_from_txt(
  FILE          *motif_file,        /* MEME file. IN */
  BUF_T         *buffer,            /* buffer. IN */
  int           num_motifs,         /* number of previously parsed motifs */
  MOTIF_T       *motifs,            /* previously parsed motifs (for the lengths) */
  STRING_LIST_T **motif_occurrences  /* List of motif occurence strings. OUT */
)
{
  ...
  *motif_occurrences = new_string_list();
  while (TRUE) {
    ...
    if (is_newline(NULL, buf_getc(buffer))) {
      //must have reached end of combined block diagram, seqid will be something like: 
      //-----------------------------------------------------------------------------
      free(seqid);
      break;
    }
    ...
    for (i = 0, unparsed = 0; diagram[i] != '\0'; ++i) {
      if (diagram[i] == '_') {
        ...
        for (m_index = 0; m_index < num_motifs; ++m_index)
          if (strcmp(motifs[m_index].id, diagram+unparsed) == 0) break;
        if (m_index < num_motifs) {
          curr_pos = seq_len;
          seq_len += motifs[m_index].length;
        } else {
...
)
{
  ...
  read_motifs(motif_file, buffer, is_html, *background, pseudocount, loading_mode, num_motifs, motifs);

  // Read the motif occurences, this is only supported for html files
  if (motif_occurrences) {
    if (is_html) {
      read_motif_occurrences_from_old_html(motif_file, buffer, motif_occurrences);
    } else {
      read_motif_occurrences_from_txt(motif_file, buffer, *num_motifs, motifs, motif_occurrences);
    }
  }
  /* Close the MEME file. */
  fclose(motif_file);
