...
struct pattern {
  ...
  int num_scanned_positions; // Count of all positions scanned for
                            // matched_element.
...
PATTERN_T *allocate_pattern(char *accession, char *name) {
  ...
  pattern->num_scanned_positions = 0;
...
void set_pattern_is_complete(PATTERN_T *pattern) {
  ...
  if (pattern->has_all_pvalues == FALSE) {
    if (verbosity >= NORMAL_VERBOSE) {
      fprintf(
	stderr,
	"Warning: due to memory limitations, matches with p-value\n"
	"greater then %0.3e have been discarded.\n",
...
/**********************************************************************
  ...
  It fails if the new max is smaller then the current number of nodes.
...
BOOLEAN_T set_pattern_max_stored_matches(PATTERN_T *pattern, int max) {
  ...
  if (pattern->max_stored_matches == max) {
    ...
    // Can't make max size of heap smaller then the current number of nodes.
    if (verbosity >= NORMAL_VERBOSE) {
      fprintf(
        stderr,
...
**********************************************************************/
int get_pattern_num_scanned_positions(PATTERN_T *pattern) {
  assert(pattern != NULL);
...
BOOLEAN_T add_pattern_matched_element(
  ...
  if (pattern->max_stored_matches > 0
      && pattern->max_stored_matches <= pattern->num_stored_matches) {
...
/**********************************************************************
  move_raw_seq_to_matched_elements

  We save store the full raw sequence in the scanned_sequences
  as long as we are keeping all matched elements. The sequence for
  a matched element is extracted on demand from the full sequence.
  However, once we start dropping matched elements to save memory
  we need to move the raw sequence data into the matched_elements themselves
  and delete the raw sequence data from the scanned sequences.
**********************************************************************/
static void move_raw_seq_to_matched_elements(PATTERN_T *pattern) {

  assert(pattern != NULL);

  // Copy the sub-sequences into the elements.
  int e_index = 0;
  for (e_index = 0; e_index < get_num_nodes(pattern->element_heap); ++e_index) {
    MATCHED_ELEMENT_T *element = (MATCHED_ELEMENT_T *) get_node(pattern->element_heap, e_index);
    // This call will actually extract the sub-sequence from the
    // parent scanned_sequence.
    char *raw_seq = get_matched_element_sequence(element);
    // Now store the raw sequence in the matched_element.
    set_matched_element_sequence(element, raw_seq);
    myfree(raw_seq);
  }

  // Now free the raw sequences from the scanned_sequences
  int seq_index = 0;
  for (seq_index = 0; seq_index < pattern->num_sequences; ++seq_index) {
    SCANNED_SEQUENCE_T *seq = pattern->sequences[seq_index];
    myfree(seq->raw_seq);
    seq->raw_seq = NULL;
  }
}
...
static void reduce_pattern_matched_elements(PATTERN_T *pattern) {
  ...
  // If this is the first call we need to move the raw sequences
  // from the scanned sequence to the matched elements
  /*
  Disabled for now
  if (!have_moved_sequences) {
    move_raw_seq_to_matched_elements(pattern);
    have_moved_sequences = TRUE;
  }
  */
...
void free_scanned_sequence(SCANNED_SEQUENCE_T *scanned_sequence) {
  ...
  myfree(scanned_sequence->raw_seq);
...
/**********************************************************************
  clear_scanned_sequence_raw_seq

  Sets the raw_seq member in a cisml scanned_sequence object to null.
**********************************************************************/
void clear_scanned_sequence_raw_seq(SCANNED_SEQUENCE_T *scanned_sequence) {

  assert(scanned_sequence != NULL);

  if (scanned_sequence->raw_seq != NULL) {
    myfree(scanned_sequence->raw_seq);
  }
  scanned_sequence->raw_seq = NULL;
...
/**********************************************************************
  has_scanned_sequence_raw_seq

  Does a scanned_sequence object have a raw seq?
**********************************************************************/
BOOLEAN_T has_scanned_sequence_raw_seq(SCANNED_SEQUENCE_T *scanned_sequence) {

  assert(scanned_sequence != NULL);

  return scanned_sequence->raw_seq != NULL ? TRUE : FALSE;

}

/**********************************************************************
  get_scanned_sequence_raw_ seq

  Retruns a copy of the raw_seq member in a cisml scanned_sequence object.
**********************************************************************/
char* get_scanned_sequence_raw_seq(SCANNED_SEQUENCE_T *scanned_sequence) {

  assert(scanned_sequence != NULL);

  return scanned_sequence->raw_seq != NULL ?
    strdup(scanned_sequence->raw_seq) : NULL;

}

/**********************************************************************
  set_scanned_sequence_raw_ seq

  Sets the raw_seq member in a cisml scanned_sequence object.
**********************************************************************/
void set_scanned_sequence_raw_seq(
  SCANNED_SEQUENCE_T *scanned_sequence,
  char *seq
) {
  ...
  scanned_sequence->raw_seq = strdup(seq);
...
**********************************************************************/
int get_scanned_sequence_num_scanned_positions(SCANNED_SEQUENCE_T *sequence) {
  assert(sequence != NULL);
...
void free_matched_element(MATCHED_ELEMENT_T *element) {
  ...
  myfree(element->score);
  myfree(element->pvalue);
  myfree(element->qvalue);
...
void set_matched_element_score(
  ...
  if (element->score == NULL) {
    element->score = mm_malloc(sizeof(double));
  }
  *(element->score) = score;
}

/**********************************************************************
  clear_matched_element_score

  Sets the score member in a cisml matched_element object to null.
**********************************************************************/
void clear_matched_element_score(MATCHED_ELEMENT_T *element) {
  assert(element != NULL);
  if (element->score != NULL) {
    myfree(element->score);
  }
...
void set_matched_element_pvalue(
  ...
  assert(element != NULL);
  if (element->pvalue == NULL) {
    element->pvalue = mm_malloc(sizeof(double));
  }
  *(element->pvalue) = pvalue;
}

/**********************************************************************
  clear_matched_element_pvalue

  Sets the pvalue member in a cisml matched_element object to null.
**********************************************************************/
void clear_matched_element_pvalue(MATCHED_ELEMENT_T *element) {
  assert(element != NULL);
  if (element->pvalue != NULL) {
    myfree(element->pvalue);
  }
...
BOOLEAN_T has_matched_element_pvalue(MATCHED_ELEMENT_T *element) {
  return element->pvalue != NULL ? TRUE : FALSE;
}
...
void set_matched_element_qvalue(
  ...
  assert(element != NULL);
  if (element->qvalue == NULL) {
    element->qvalue = mm_malloc(sizeof(double));
  }
  *(element->qvalue) = qvalue;
}

/**********************************************************************
  clear_matched_element_qvalue

  Sets the qvalue member in a cisml matched_element object to null.
**********************************************************************/
void clear_matched_element_qvalue(MATCHED_ELEMENT_T *element) {
  assert(element != NULL);
  if (element->qvalue != NULL) {
    myfree(element->qvalue);
  }
...
BOOLEAN_T has_matched_element_qvalue(MATCHED_ELEMENT_T *element) {
  return element->qvalue != NULL ? TRUE : FALSE;
}
...
**********************************************************************/
char *get_matched_element_sequence(MATCHED_ELEMENT_T* element) {

  assert(element != NULL);

  char *raw_seq = NULL;
  if (element->sequence != NULL) {
    // The raw sub-sequence has been stored in the matched element.
    raw_seq = strdup(element->sequence);
  }
  else {
    // The raw sequence remains in the sequence.
    // Extract the portion corresponging to the matched element.
    SCANNED_SEQUENCE_T *parent_sequence = element->parent_sequence;
    assert(parent_sequence != NULL);
    assert(parent_sequence->raw_seq != NULL);
    int start = MIN(element->start, element->stop);
    int stop = MAX(element->start, element->stop);
    int length = stop - start + 1;

    raw_seq = mm_malloc(sizeof(char) * (length + 1));
    strncpy(raw_seq, (parent_sequence->raw_seq + start - 1), length);
    raw_seq[length] = 0; // Add terminating null character.
  }
...
void print_cisml_matched_elements(
  ...
  for(i_element = 0; i_element < num_matched_elements; i_element++) {
    ...
    fprintf(out, ">\n");
    char *sequence = get_matched_element_sequence(element);
    if (sequence !=  NULL) {
...
void read_patterns_from_cisml(
  ...
  *num_patterns = (xpath_obj->nodesetval ? xpath_obj->nodesetval->nodeNr : 0);
  ...
  for (i_pattern = 0; i_pattern < *num_patterns; i_pattern++) {
	  ...
	  // check if at least on sequence has been assigned
	  int num_seq = 0;
 ...
 * matched elements.
 *************************************************************************/
void pattern_calculate_qvalues(PATTERN_T *pattern) {

  assert(pattern != NULL);
  assert(pattern->is_complete == TRUE);

  int num_stored_matches = get_pattern_num_stored_matches(pattern);
  int num_scanned_positions = get_pattern_num_scanned_positions(pattern);

  // Tell the user what's up.
  if (verbosity >= NORMAL_VERBOSE) {
    fprintf(
	    stderr,
	    "Computing q-values (%d recorded p-values, %d total p-values).\n",
	    num_stored_matches,
  ...
  if (num_stored_matches) {
    ...
    //  We can only calculate pi0 if we have all p-values.
    BOOLEAN_T use_pi0 = get_pattern_has_all_pvalues(pattern);
