...
static const char *HTML_STYLESHEET = "gomo.xsl";
 ...
 */
inline int tuple_pvalue_compar(const void *v1, const void *v2) 
{
 ...
 * score.
 */
inline int bin_bsearch_compar(const void *key, const void *entry) {
 ...
 */
inline int bins_bsearch_compar(const void *key, const void *entry) {
 ...
 */
inline int bounded_random(int max) 
{
  assert(max < RAND_MAX);
  int bound = RAND_MAX;
...
inline double get_ama_score(const void *scores_ptr, int index) {
  AMA_LIST_T *scores = (AMA_LIST_T*)scores_ptr;
  return scores->list[index].score; 
}
inline BOOLEAN_T get_false(const void *ignore1, int ignore2) {
  return FALSE; 
}
    ...
    ) 
{
  ...
  *seq_ids = rbtree_create(str_compar,NULL,free,int_copy,free);
  ...
  *gene_url = buf_fread_token(buffer, fp, is_newline, NULL, FALSE, NULL, 0, NULL);
  ...
  while (!feof(fp) || buf_remaining(buffer)) {
    ...
    if (newline) {//new line was previous char
      ...
      if (term_loading_num > 0) {
        ...
        arraylst_clear(term_seq_load_nums);
      }
      //lookup the term id
  ...
  //cleanup
  fclose(fp);
  arraylst_destroy(term_seq_load_nums);
...
int main(int argc, char *argv[]) 
{
  ...
  //destroy file lists
  arraylst_destroy(ama_paths);
  
  if (filter_motifs != NULL) arraylst_destroy(filter_motifs);
