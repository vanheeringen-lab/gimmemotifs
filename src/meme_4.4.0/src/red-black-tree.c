 ...
 */
int check_recursive(RBNODE_T *node, BOOLEAN_T must_be_black, int (*cmp)(void*, void*), int *black_nodes_to_leaf) {
 ...
 *  A   B                         B   C
 */
inline void rotate_tree_right(RBTREE_T *tree, RBNODE_T *x) {
  RBNODE_T *y;
  assert(x != NULL);
  y = x->left;
  assert(y != NULL);
 ...
 *      B   C                 A   B
 */
inline void rotate_tree_left(RBTREE_T *tree, RBNODE_T *x) {
  RBNODE_T *y;
  assert(x != NULL);
  y = x->right;
  assert(y != NULL);
 ...
 */
inline void fix_consecutive_reds(RBTREE_T *tree, RBNODE_T *node) {
 ...
 * prior to deletion.
 */
inline void fix_double_black(RBTREE_T *tree, RBNODE_T *double_black_node) {
 ...
 */
void delete_internal(RBTREE_T *tree, RBNODE_T *node) {
