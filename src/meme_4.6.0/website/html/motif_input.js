...
var log_box = null;
var vis_ids = [];
var motif_change_callback = null;
...
function setup_motif_input() {
  ...
  if (document.getElementById('type_s').checked) motif_input_format('consensus');
  else if (document.getElementById('type_f').checked) motif_input_format('matrix');
  else if (document.getElementById('type_m').checked) motif_input_format('meme');
  else {
    var inline_motif = document.getElementById('type_i');
    if (inline_motif != null && inline_motif.checked) motif_input_format('inline');
  }
  update_preview();
}
...
//
function reset_motif_input() {
  // if there was a inline motif then select it by default
  if (document.getElementById('type_i') == null) motif_input_format('consensus');
  else motif_input_format('inline');
...
function check_motif_input() {
  ...
  if (document.getElementById('type_m').checked) {
    ...
    if (logo_warning_count > 0) {
      ...
      if (!ignore_warnings) {
        select_motif_input_preview_tab(false);
        return false;
      }
    }
  }
...
//
// set_motif_change_callback
//
function set_motif_change_callback(callback) {
  motif_change_callback = callback;
}

function update_motif_count() {
  var count = 1; 
  var //TODO fix me
  if (document.getElementById('type_m').checked) {
    if (document.getElementById('m_all').checked) {
      ...
      var filter = document.getElementById('m_filter');
      var items = filter.split(/\s+/);
      if (items.length > 0 && items[0] == "") items.shift();
...
//
function motif_input_format(format) {
...
//
// select_motif_input_preview_tab
//
function select_motif_input_preview_tab(is_logo) {
  if (is_logo) {
...
function update_preview() {
  ...
  if (preview_support && logo_warning_count == 0 && logo_error_count == 0) select_motif_input_preview_tab(true);
}
