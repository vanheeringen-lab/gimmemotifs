...
def write_shell_script(tool, fastafile, rundir=os.path.abspath("."), params=None):
    ...
    if params["background"]:
        tmp.write(
            "cp {0} {1}/{2}\n".format(
                os.path.abspath(params["background"]), tmpdir, bgfa
            )
        )
        ...
        if t.name in tools and tools[t.name]:
            if t.use_width:
                for i in range(wmin, wmax + 1, step):
                    ...
                    params["width"] = i
                    sh = write_shell_script(
                        t.name, fastafile, rundir=rundir, params=params
                    )
                ...
                if not job:  # or not job.is_running():
                    ...
                    if job:
                        ...
                        if os.path.exists(pwmfile):
                            ...
                            logger.error(
                                "Job {0} finished, but couldn find {1}!".format(
                                    job_name, pwmfile
                                )
                            )
                    stdout = open(
                        os.path.join(rundir, "{0}.stdout".format(job_name))
                    ).read()
                    stderr = open(
                        os.path.join(rundir, "{0}.stderr".format(job_name))
                    ).read()
