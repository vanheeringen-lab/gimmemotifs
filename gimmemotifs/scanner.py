...
def load_motifs(motif_file):
    ...
    cutoffs = parse_cutoff(motifs, 0.95)
    cutoffs = [cutoffs[m.id] for m in motifs]
...
def scan_regions(regions, index_dir, motif_file, nreport, scan_rc, use_cache=True):
    ...
    if use_cache:
        ...
        for region in regions:
            key = str((region, index_dir, motif_file, nreport, scan_rc))
            ret = cache.get(key)
            if ret == NO_VALUE:
...
def scan_sequences(seqs, motif_file, nreport, scan_rc, use_cache=True):
    ...
    if use_cache:
        ...
        for seq,seq_hash in hashes.items():
            key = str((seq_hash, motif_file, nreport, scan_rc))
            ret = cache.get(key)
            if ret == NO_VALUE:
        ...
        for job in jobs:
            for ret in job.get():
                if use_cache:
                    ...
                    key = str((h, motif_file, nreport, scan_rc))
                    cache.set(key, ret)
                else: 
                    yield ret
                i += 1
    ...
    if use_cache:
        # return results from cache
        for seq in seqs:
            key = str((hashes[seq.upper()], motif_file, nreport, scan_rc))
            ret = cache.get(key)
            yield ret
...
class Scanner(object):
    """
    scan fasta sequences with motifs
    """
    ...
    def scan(self, seqs, nmax=1, scanrc=True):
        """
